...
| 60 | OTel observability (opt-in)      | FULLY_FUNCTIONAL     | `pkg/cqrs` + `pkg/api`  | `CQRSConfig.OTel *middleware.OTelBundle` attaches command/event middleware (spans + `cqrs.operation.*` metrics), opens a `localsync.sync_items` span per batch, and feeds the projection host an adapter recorder (event/dead-letter/worker/checkpoint events). `api.WithMetricsHandler` mounts any exporter (e.g. promhttp) under `GET /metrics`. Nil = off, zero behavior change.                                                                |
...
| 62 | API rate limiting                | FULLY_FUNCTIONAL     | `pkg/api`               | `WithRateLimit(perMinute)`: token bucket guarding `POST /sync` only; 429 + `Retry-After` + JSON body. Reads unlimited. Off by default.                                                                                                                                                                                                                                                                                                             |
...
| 67 | localsync-lint CLI + tests       | FULLY_FUNCTIONAL     | `cmd/localsync-lint`    | CLI for the internal linter: `--strict`, `--verbose`, `--show-suppressed`, `--json` (with suppression provenance). 8 tests: exit-code decision table, summary/JSON schema, verbose rule status, violating-fixture + suppression round trip.                                                                                                                                                                                                        |
