...
- **Private dependency**: `go-cqrs-lite` is a **private** GitHub repo (siblings `go-branded-id` and `go-error-family` are public). The nix sandbox cannot fetch it, so `nix build` / `nix flake check` require **vendored deps**: a committed `vendor/` dir + `vendorHash = null` in `flake.nix` + a `vendor/**` exclusion in `treefmt`. Regenerate with `GOWORK=off go mod vendor`. **Critical**: `.gitignore` contains `vendor/`, so after re-vendoring you MUST force-add the `larsartmann/*` deps (`git add -f vendor/github.com/larsartmann/`) — nix flakes only include git-tracked files in the source, so untracked vendor files are silently dropped from the sandbox and `nix build` fails with `cannot find module providing package ... import lookup disabled by -mod=vendor`. The other (public) vendor deps are already force-tracked. Cleanest long-term fix: make `go-cqrs-lite` public, then switch to a real `vendorHash` and drop `vendor/`. The same force-add pattern applies to **`.envrc`** (committed so direnv auto-loads the devShell): buildflow's `gitignore-upserter` recommends ignoring it, so it lives in the buildflow-managed `.gitignore` block. After creating/modifying it, force-add once with `git add -f .envrc`; once tracked, `.gitignore` no longer hides it and subsequent edits work normally.
