...
| ------------------- | ----- | -------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `pkg/cqrs`          | 150   | 87.7%    | ✅ Decider, ReadModel, Projection, Stack, SQLite RM, Replay, Correlation, tombstone, upcasting, scenario specs, regression tests |
...
**320 total test functions** across 11 test packages (incl. `cmd/cqrs-lint`), plus 31 in the standalone `provider/github` module; the whole suite is race-clean.
