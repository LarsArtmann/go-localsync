	...
	Describe("as a developer building an offline-first dashboard", func() {
		Context("when I store GitHub events", func() {
			BeforeEach(func() {
				...
				world.items = []*provider.Item{
					newStorageItem("1", "PushEvent", "alice", "alice/repo1", now.Add(-3*time.Hour)),
					newStorageItem("2", "IssuesEvent", "bob", "bob/repo2", now.Add(-2*time.Hour)),
					newStorageItem("3", "PullRequestEvent", "alice", "alice/repo1", now.Add(-1*time.Hour)),
				}
			})
		...
		Context("when I store the same event twice (idempotency)", func() {
			BeforeEach(func() {
				...
				world.item = newStorageItem("duplicate-id", "PushEvent", "user", "repo", time.Now())
			})
		...
		Context("when I query for the latest event", func() {
			BeforeEach(func() {
				...
				now := time.Now()
				_ = world.store.Upsert(world.ctx, newStorageItem("old", "PushEvent", "user", "repo", now.Add(-2*time.Hour)))
				_ = world.store.Upsert(world.ctx, newStorageItem("middle", "IssuesEvent", "user", "repo", now.Add(-1*time.Hour)))
				_ = world.store.Upsert(world.ctx, newStorageItem("newest", "PullRequestEvent", "user", "repo", now))
			})
		...
		Context("when I filter events by type", func() {
			BeforeEach(func() {
				...
				now := time.Now()
				_ = world.store.Upsert(world.ctx, newStorageItem("1", "PushEvent", "alice", "repo1", now))
				_ = world.store.Upsert(world.ctx, newStorageItem("2", "IssuesEvent", "bob", "repo2", now))
				_ = world.store.Upsert(world.ctx, newStorageItem("3", "PushEvent", "charlie", "repo3", now))
				_ = world.store.Upsert(world.ctx, newStorageItem("4", "PullRequestEvent", "alice", "repo1", now))
			})
		...
		Context("when I filter events by actor", func() {
			BeforeEach(func() {
				...
				_ = world.store.Upsert(world.ctx, newStorageItem("1", "PushEvent", "alice", "repo1", now))
				_ = world.store.Upsert(world.ctx, newStorageItem("2", "IssuesEvent", "bob", "repo2", now))
				_ = world.store.Upsert(world.ctx, newStorageItem("3", "PushEvent", "alice", "repo3", now))
			})
		...
		Context("when I filter events by repository", func() {
			BeforeEach(func() {
				...
				now := time.Now()
				_ = world.store.Upsert(world.ctx, newStorageItem("1", "PushEvent", "alice", "owner/repo-a", now))
				_ = world.store.Upsert(world.ctx, newStorageItem("2", "IssuesEvent", "bob", "owner/repo-b", now))
				_ = world.store.Upsert(world.ctx, newStorageItem("3", "PushEvent", "charlie", "owner/repo-a", now))
			})
		...
		Context("when I request statistics", func() {
			BeforeEach(func() {
				...
				now := time.Now()
				_ = world.store.Upsert(world.ctx, newStorageItem("1", "PushEvent", "alice", "repo", now))
				_ = world.store.Upsert(world.ctx, newStorageItem("2", "PushEvent", "bob", "repo", now))
				_ = world.store.Upsert(world.ctx, newStorageItem("3", "IssuesEvent", "alice", "repo", now))
				_ = world.store.Upsert(world.ctx, newStorageItem("4", "PullRequestEvent", "bob", "repo", now))
			})
