...
This was the exact bug we fixed. But it highlights that the ROADMAP was written when this was considered a *design question*, not a *bug*. The fact that it was listed as an open question instead of a known bug suggests the original audit missed the severity.
