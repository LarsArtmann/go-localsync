...
ConflictAwareSyncer error-drop fixed (ErrPartialSync, v0.4.0); CI pipeline exists (reworked v0.4.0, security+provider legs added by v0.5.0); the replace-directive problem died with the v2 migration; dispatcher/middleware/LWWResolver shipped by v0.4.0; metrics ideas routed to ROADMAP. No live items remain.
