...
Era-closed: the S/A/B/C tier plan targets the pre-rewrite layout; ID consolidation → shipped differently (branded `pkg/id`, ADR-0002); exhaustruct discipline → enforced by the pinned linter config today. No live items remain here; the living trackers are [TODO_LIST.md](../../../TODO_LIST.md) and [ROADMAP.md](../../../ROADMAP.md).
