...
Both CRITICAL findings (split-brain conflict detection, no projection replay) were fixed within the day (see the 06-42/21-29 reports); the adoption items (JSONCodec, NewEvents, correlation, snapshots, LWWResolver) shipped through v0.1.0-v0.4.0; Turso items moot (v0.2.0); the README test-count item is kept fresh by the docs-health passes. No live items remain.
