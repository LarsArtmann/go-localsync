...
- ~~[ ] **Integration test for full sync pipeline** — Provider → CQRS → read model → API round-trip. No end-to-end test exists.~~ → false today — full-pipeline integration tests + benchmarks shipped (M10/M27)
...
- **Shipped since:** TypedItem and the query packages were deleted; full-pipeline integration + concurrent read model tests shipped; CLI coverage became moot when cmd/ was removed.
