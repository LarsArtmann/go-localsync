...
Executed: projection runner + outbox publisher + correlation IDs + typed middleware shipped 2026-05-21/22 (Turso machinery later deleted in v0.2.0). T10 (`middleware.CommandRetry` adoption) was never done and is now moot — retry lives in `pkg/sync/retry.go` and `provider/github` (v0.5.0). No live items remain.
